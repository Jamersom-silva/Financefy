...
export default function Attachments() {
  ...
  const loadGroups = useCallback(async () => {
    setGroups(await apiGet<Group[]>("/attachment-groups/"));
  }, []);

  // ============================================================
  // 🔹 Carregar registros (meses/anos) do grupo selecionado
  // ============================================================
  const loadRecords = useCallback(async () => {
    if (!expandedGroup) return;
    setRecords(
      await apiGet<RecordItem[]>(`/attachment-records/?group=${expandedGroup}`)
    );
  }, [expandedGroup]);

  // ============================================================
  // 🔹 Carregar anexos do registro selecionado
  // ============================================================
  const loadAttachments = useCallback(async () => {
    if (!expandedRecord) return;

    const data = await apiGet<Attachment[]>(
      `/attachments/?record=${expandedRecord}`
    );
  ...
  const createGroup = async () => {
    ...
    const newGroup = await apiRequest<Group>(
      "/attachment-groups/",
      "POST",
      { name: newGroupName }
    );
  ...
  // ============================================================
  // 🔹 Criar novo registro (mês/ano)
  // ============================================================
  const createRecord = async (groupId: number) => {
    const newRecord = await apiRequest<RecordItem>(
      "/attachment-records/",
      "POST",
  ...
  const upload = async (type: "invoice" | "receipt") => {
    if (!selectedRecord)
      return toast.error("Selecione um mês antes de enviar.");
